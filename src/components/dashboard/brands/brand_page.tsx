...
import BrandModalForm from './modal/brand-form';
...
export function BrandPage(): React.JSX.Element {
  ...
  const fetchbrandData = async () => {
    ...
    fetch(`${baseUrl}/api/brand?page=${page + 1}&limit=${rowsPerPage}&search=${search}`,
      {
        method: 'GET',
        headers: {
  ...
  const handleDeletebrand = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/brand/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${authClient.getToken()}`
          }
  ...
  return (
    <>
      <BrandModalForm open={open} handleClose={handleModalClose} brandID={brandID} fetchbrandData={fetchbrandData} />
                ...
                return (
                  <TableRow hover key={row.id}>
                    ...
                    <TableCell>
                      ...
                      {userData?.role === "Admin" || userData?.role === "SuperAdmin" ? <IconButton color='error' onClick={() => handleDeletebrand(row.id)}> <Delete /> </IconButton> : null}
                    </TableCell>
                  </TableRow>
                );
