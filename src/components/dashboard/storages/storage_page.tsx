...
import StorageModalForm from './modal/storage-form';
...
export function StoragePage(): React.JSX.Element {
  ...
  const [rows, setRows] = React.useState<Storage[]>([]);
  const [search, setSearch] = React.useState<string>('');
  const [count, setCount] = React.useState(0);
  const [storageID, setstorageID] = React.useState(0);
  ...
  const fetchstorageData = async () => {
    ...
    fetch(`${baseUrl}/api/storage?page=${page + 1}&limit=${rowsPerPage}&search=${search}`,
      {
        method: 'GET',
        headers: {
  ...
  const handleEditstorage = (id: number) => {
    setstorageID(id)
    handleOpen()
  }

  const handleEditstoragePassword = (id: number) => {
    setstorageID(id)
  ...
  const handleDeletestorage = (id: number) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/storage/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${authClient.getToken()}`
          }
  ...
  return (
    <>
      <StorageModalForm open={open} handleClose={handleModalClose} storageID={storageID} fetchStorageData={fetchstorageData} />
      <Stack direction="row" spacing={3}>
        <Stack spacing={1} sx={{ flex: '1 1 auto' }}>
          ...
          <Stack direction="row" spacing={1} sx={{ alignItems: 'center' }}>
            ...
            />
            <Button color="inherit" startIcon={<Refresh />} sx={{ bgcolor: '#f6f9fc' }} onClick={fetchstorageData}>
              refresh
            </Button>
          </Stack>
        </Stack>
              ...
              {rows.length > 0 ? rows.map((row) => {
                return (
                  <TableRow hover key={row.id}>
                    ...
                    <TableCell>
                      <IconButton color='warning' onClick={() => handleEditstorage(row.id)}>
                        <Edit />
                      </IconButton>
                      <IconButton color='error' onClick={() => handleDeletestorage(row.id)}>
                        <Delete />
                      </IconButton>
                    </TableCell>
                  </TableRow>
